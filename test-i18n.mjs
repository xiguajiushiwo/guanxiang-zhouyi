...
assert.equal(t('liuyao.mode'), 'ناجیای شش‌خطی');
assert.equal(t('liuyao.currentRestored'), 'زمان فعلی این دستگاه بازیابی شد.');
assert.equal(t('liuyao.historyTitle'), 'بازسازی‌شده از زمان فال');
assert.equal(t('liuyao.legacyMissing'), 'این رکورد قدیمی پیش از افزوده‌شدن جدول شش‌خطی ساخته شده است؛ تفسیر کلاسیک آن کامل مانده است.');
setLanguage('en');
