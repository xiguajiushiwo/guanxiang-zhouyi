...
function renderHistory(){
  ...
  list.innerHTML=visible.map(record=>{const original=hexagrams[record.originalIndex],changed=hexagrams[record.changedIndex],date=new Date(record.completedAt),dateText=Number.isNaN(date.getTime())?t('history.timeMissing'):date.toLocaleString(dateLocale,{year:'numeric',month:english?'short':'long',day:'numeric',hour:'2-digit',minute:'2-digit'}),name=english?`${displayHexagramName(record.originalIndex,'en',original?.[0])} → ${displayHexagramName(record.changedIndex,'en',changed?.[0])}`:`${original?.[2]||'本卦'}之${changed?.[2]||'变卦'}`;return `<button type="button" class="history-item" data-history-id="${attributeHtml(record.id)}"><span class="history-glyph">${original?.[1]||'—'}<i>→</i>${changed?.[1]||'—'}</span><span class="history-item-copy"><b data-user-content>${textHtml(record.question)}</b><small>${textHtml(name)}${record.note?` · ${t('history.hasNote')}`:''}</small></span><time>${textHtml(dateText)}</time><span class="history-item-arrow" aria-hidden="true">→</span></button>`}).join('');
  list.querySelectorAll('.history-item').forEach(button=>button.addEventListener('click',()=>{historySelectedId=button.dataset.historyId;nav('history');renderHistory()}));
}
...
function renderHistoryInterpretation(detail,reading,cached){
  ...
  if(cached?.text){const ai=document.createElement('section');ai.className='interpretation-band ai-interpretation history-interpretation';ai.innerHTML=`<div class="interpretation-heading"><div><span class="panel-kicker">${t('reading.ai')}</span><h3>${t('reading.aiSaved')}</h3></div><time>${textHtml(new Date(cached.generatedAt).toLocaleString(getLanguage()==='en'?'en-US':'zh-CN'))}</time></div><div class="interpretation-sections ai-reading-content">${aiReadingBodyHtml(cached.text)}</div>`;anchor.appendChild(ai)}
}
