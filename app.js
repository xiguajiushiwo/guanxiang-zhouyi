...
function lineName(value){const language=getLanguage();if(language==='en')return value===6?'Old yin · changing':value===9?'Old yang · changing':value===8?'Young yin':'Young yang';if(language==='fa')return value===6?'یین پیر · متغیر':value===9?'یانگ پیر · متغیر':value===8?'یین جوان':'یانگ جوان';return value===6?'老阴 · 变':value===9?'老阳 · 变':value===8?'少阴':'少阳'}
function renderRecords(){const rec=$('#lineRecords');$('#recordEmpty').classList.toggle('hidden',castState.lines.length>0);rec.innerHTML=castState.lines.map((line,i)=>{const path=line.changes?.length?`<em>${line.changes.map(change=>change.remaining).join(' → ')} ÷ 4</em>`:'';return `<div class="line-record"><strong>${line.value===6?'⚋ ×':line.value===9?'⚊ ○':line.value===8?'⚋':'⚊'}</strong><span>${liuyaoLineLabel(i+1)} · ${lineName(line.value)}${path}</span><small>${line.value}</small></div>`}).join('')}
...
function ensureCastTimePanel(){
  if($('#castTimePanel'))return;
  const compose=$('.question-compose'),confirm=compose?.querySelector('.question-confirm');
  if(!compose||!confirm)return;
  const panel=document.createElement('fieldset');
  panel.id='castTimePanel';panel.className='cast-time-panel';
  panel.innerHTML=`<legend>${t('liuyao.castTime')}</legend><div class="cast-time-summary"><span id="castTimeSummary">${t('liuyao.defaultTime')}</span><button type="button" class="text-button" id="useCurrentCastTime">${t('liuyao.useCurrent')}</button></div><div class="cast-time-fields"><label>${t('liuyao.date')}<input id="castDate" type="date" /></label><label>${t('liuyao.time')}<input id="castClock" type="time" step="60" /></label><label>${t('liuyao.timeZone')}<input id="castTimeZone" type="text" list="timeZoneSuggestions" autocomplete="off" /></label></div><datalist id="timeZoneSuggestions"><option value="Asia/Shanghai"></option><option value="Asia/Tehran"></option><option value="Asia/Tokyo"></option><option value="Europe/London"></option><option value="America/New_York"></option><option value="UTC"></option></datalist><p class="cast-time-feedback" id="castTimeFeedback" aria-live="polite">${t('liuyao.timeHint')}</p>`;
  compose.insertBefore(panel,confirm);
}
function deviceCastTime(){return normalizeCastTime({value:new Date(),timeZone:Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'})}
function renderCastTimeControls(meta=null){
  const panel=$('#castTimePanel');if(!panel)return;
  let normalized=meta;
  try{normalized=normalized||deviceCastTime()}catch{normalized={iso:new Date().toISOString(),timeZone:'UTC',localDateTime:new Date().toISOString().slice(0,16)}}
  const [date,clock]=normalized.localDateTime.split('T');
  $('#castDate').value=date;$('#castClock').value=clock;$('#castTimeZone').value=normalized.timeZone;
  panel.querySelector('legend').textContent=t('liuyao.castTime');$('#useCurrentCastTime').textContent=t('liuyao.useCurrent');
  $('#castTimeSummary').textContent=castState.confirmed?t('liuyao.lockedTime'):`${t('liuyao.defaultTime')} · ${normalized.timeZone}`;
  $('#castTimeFeedback').textContent=castState.confirmed?t('liuyao.timeLocked'):t('liuyao.timeHint');
  panel.classList.toggle('locked',Boolean(castState.confirmed));
  panel.querySelectorAll('input,button').forEach(control=>{control.disabled=Boolean(castState.confirmed)});
}
function readCastTimeForm(){
  const date=$('#castDate')?.value,clock=$('#castClock')?.value,zone=$('#castTimeZone')?.value?.trim();
  if(!date||!clock||!zone)throw new RangeError('请填写完整的起卦时间、日期和时区。');
  const iso=localDateTimeToIso(`${date}T${clock}`,zone);
  return normalizeCastTime({value:iso,timeZone:zone});
}
function useCurrentCastTime(){const meta=deviceCastTime();renderCastTimeControls(meta);$('#castTimeFeedback').textContent=t('liuyao.currentRestored')}
...
function saveCast(){try{writeJson(localStorage,'guanxiang-cast-v3',{version:3,savedAt:new Date().toISOString(),sessionId:castState.sessionId,question:castState.question,confirmed:castState.confirmed,prepared:castState.prepared,mode:castState.mode,castAt:castState.castAt,timeZone:castState.timeZone,lines:castState.lines,ritual:castState.ritual})}catch(error){console.warn('起卦进度未能保存',error)}}
...
function saveReadingHistory(originalIndex,changedIndex,moving,liuyao=null){if(!castState.sessionId||castState.lines.length!==6||!castState.lines.every(validLine))return;try{const records=loadHistory(),stamp=new Date().toISOString(),record={id:castState.sessionId,completedAt:stamp,createdAt:stamp,updatedAt:stamp,question:castState.question,lines:castState.lines,originalIndex,changedIndex,moving,mode:castState.mode,castingMethod:'yarrow',castAt:castState.castAt||stamp,timeZone:castState.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC',tags:[],reviewState:'未开始',...(liuyao?{liuyao}:{})};const existing=records.findIndex(item=>item.id===record.id);if(existing>=0)records[existing]={...records[existing],...record,note:records[existing].note||'',createdAt:records[existing].createdAt||record.createdAt};else records.unshift({...record,note:''});const saved=records.find(item=>item.id===record.id);persistHistory(records);syncCloudRecord(saved);renderBackupReminder();if(currentView==='history')renderHistory()}catch(error){console.warn('占问记录未能保存',error)}}
...
function renderHistoryDetail(record){
  ...
  const date=new Date(record.completedAt),language=getLanguage(),english=language==='en',locale=language==='fa'?'fa-IR':english?'en-US':'zh-CN';
  const positions=language==='zh-CN'?['初','二','三','四','五','上']:Array.from({length:6},(_,index)=>liuyaoLineLabel(index+1));
  ...
  if(record.liuyao){const liuyaoSection=document.createElement('section');liuyaoSection.className='history-liuyao';liuyaoSection.innerHTML=`<div class="liuyao-reading-head"><div><span class="panel-kicker">${t('liuyao.history')}</span><h3>${t('liuyao.historyTitle')}</h3></div></div><div data-history-liuyao></div>`;detail.querySelector('.history-interpretation-anchor')?.before(liuyaoSection);liuyaoSection.querySelector('[data-history-liuyao]').innerHTML=liuyaoMarkup(record.liuyao);liuyaoSection.querySelector('#liuyaoYongShen')?.addEventListener('change',event=>{const records=loadHistory(),stored=records.find(item=>item.id===record.id);if(!stored?.liuyao)return;stored.liuyao=applyYongShenOverride(stored.liuyao,event.target.value);stored.updatedAt=new Date().toISOString();persistHistory(records);syncCloudRecord(stored);renderHistoryDetail(stored)})}
  else {const missing=document.createElement('p');missing.className='history-liuyao-missing';missing.textContent=t('liuyao.legacyMissing');detail.querySelector('.history-interpretation-anchor')?.before(missing)}
...
function ensureReadingModes(){
  const panel=$('#readingPanel');if(!panel||$('#readingModeTabs'))return;
  const local=$('#localReading'),ai=$('#aiReading'),columns=$('#readingColumns'),related=panel.querySelector('.reading-related'),note=$('#resultNote');
  if(!local||!ai||!columns||!related||!note)return;
  const tabs=document.createElement('div');tabs.id='readingModeTabs';tabs.className='reading-mode reading-result-modes';tabs.setAttribute('role','tablist');tabs.innerHTML=`<button type="button" role="tab" aria-selected="true" data-reading-result-mode="classic">${t('liuyao.classicMode')}</button><button type="button" role="tab" aria-selected="false" data-reading-result-mode="liuyao">${t('liuyao.mode')}</button>`;
  const classic=document.createElement('div');classic.id='classicReadingMode';classic.className='reading-result-mode';
  local.parentNode.insertBefore(classic,local);classic.append(local,ai,columns,related,note);
  const liuyao=document.createElement('section');liuyao.id='liuyaoReadingMode';liuyao.className='reading-result-mode hidden';liuyao.innerHTML=`<div class="liuyao-reading-head"><div><span class="panel-kicker">${t('liuyao.laterSystem')}</span><h3>${t('liuyao.completeChart')}</h3></div><span class="interpretation-source">${t('liuyao.reviewable')}</span></div><div id="liuyaoReadingContent"></div>`;
  classic.after(liuyao);panel.insertBefore(tabs,panel.querySelector('.reading-question'));
  tabs.querySelectorAll('[data-reading-result-mode]').forEach(button=>button.addEventListener('click',()=>setReadingResultMode(button.dataset.readingResultMode)));
}
function setReadingResultMode(mode='classic'){
  ensureReadingModes();const classic=$('#classicReadingMode'),liuyao=$('#liuyaoReadingMode');if(!classic||!liuyao)return;
  const isLiuyao=mode==='liuyao';classic.classList.toggle('hidden',isLiuyao);liuyao.classList.toggle('hidden',!isLiuyao);
  document.querySelectorAll('[data-reading-result-mode]').forEach(button=>{const active=(button.dataset.readingResultMode===mode);button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active))});
}
function localizeReadingModes(){
  const classic=$('[data-reading-result-mode="classic"]'),liuyao=$('[data-reading-result-mode="liuyao"]'),head=$('#liuyaoReadingMode .liuyao-reading-head');
  if(classic)classic.textContent=t('liuyao.classicMode');
  if(liuyao)liuyao.textContent=t('liuyao.mode');
  if(head){const kicker=head.querySelector('.panel-kicker'),title=head.querySelector('h3'),source=head.querySelector('.interpretation-source');if(kicker)kicker.textContent=t('liuyao.laterSystem');if(title)title.textContent=t('liuyao.completeChart');if(source)source.textContent=t('liuyao.reviewable')}
}
const LIUYAO_TERMS={en:{父母:'Parent',兄弟:'Sibling',子孙:'Child',妻财:'Wealth',官鬼:'Officer',世爻:'Self line',应爻:'Responding line',青龙:'Azure Dragon',朱雀:'Vermilion Bird',勾陈:'Hooked Array',螣蛇:'Flying Serpent',白虎:'White Tiger',玄武:'Dark Warrior',木:'Wood',火:'Fire',土:'Earth',金:'Metal',水:'Water',旺:'Prosperous',相:'Supported',休:'Resting',囚:'Confined',死:'Weak',六合:'Combine',六冲:'Clash',六害:'Harm',相刑:'Punishment',相破:'Break',半合水局:'Partial Water combination',半合木局:'Partial Wood combination',半合火局:'Partial Fire combination',半合金局:'Partial Metal combination',化回头生:'Transforms to generate back',化回头克:'Transforms to control back',化进神:'Advancing transformation',化退神:'Retreating transformation',化合:'Transforms to combine',化冲:'Transforms to clash'},fa:{父母:'والد',兄弟:'هم‌رتبه',子孙:'فرزند',妻财:'ثروت',官鬼:'مقام',世爻:'خط خود',应爻:'خط پاسخ',青龙:'اژدهای لاجوردی',朱雀:'پرنده سرخ',勾陈:'آرایش قلابی',螣蛇:'مار پرنده',白虎:'ببر سفید',玄武:'جنگجوی تیره',木:'چوب',火:'آتش',土:'خاک',金:'فلز',水:'آب',旺:'نیرومند',相:'پشتیبانی‌شده',休:'آرام',囚:'محصور',死:'ضعیف',六合:'پیوند',六冲:'برخورد',六害:'آسیب',相刑:'کیفر',相破:'گسست',半合水局:'نیم‌پیوند آب',半合木局:'نیم‌پیوند چوب',半合火局:'نیم‌پیوند آتش',半合金局:'نیم‌پیوند فلز',化回头生:'دگرگونی و زایش بازگشتی',化回头克:'دگرگونی و مهار بازگشتی',化进神:'دگرگونی پیش‌رونده',化退神:'دگرگونی پس‌رونده',化合:'دگرگونی به پیوند',化冲:'دگرگونی به برخورد'}};
function liuyaoTerm(value){return LIUYAO_TERMS[getLanguage()]?.[value]||value}
function liuyaoLineLabel(position){if(getLanguage()==='en')return `Line ${position}`;if(getLanguage()==='fa')return `خط ${position}`;return ['初爻','二爻','三爻','四爻','五爻','上爻'][position-1]||`${position}爻`}
function liuyaoRelationMarkup(relation){const language=getLanguage(),target=relation.target==='月建'?(language==='fa'?'ماه':'Month'):relation.target==='日辰'?(language==='fa'?'روز':'Day'):relation.withPosition?liuyaoLineLabel(relation.withPosition):(language==='fa'?'رابطه':'Relation'),description=language==='zh-CN'?(relation.status||`${relation.target||'目标'}关系`):target;return `<li><b>${textHtml(liuyaoTerm(relation.type))}</b><span>${textHtml(description)}</span></li>`}
function liuyaoMarkup(chart){
  if(!chart)return `<div class="liuyao-unavailable"><b>${t('liuyao.unavailable')}</b><p>${t('liuyao.unavailableHint')}</p></div>`;
  const language=getLanguage(),locale=language==='fa'?'fa-IR':language==='en'?'en-US':'zh-CN',date=new Date(chart.castAt),dateText=Number.isNaN(date.getTime())?chart.localDateTime||chart.castAt:date.toLocaleString(locale,{year:'numeric',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
  const rows=chart.lines.slice().reverse().map(line=>`<tr><th>${textHtml(liuyaoLineLabel(line.position))}</th><td>${textHtml(line.ganzhi)}<small>${textHtml(liuyaoTerm(line.element))}</small></td><td>${textHtml(liuyaoTerm(line.relative)||'—')}</td><td>${textHtml(liuyaoTerm(line.spirit))}</td><td>${line.shi?textHtml(liuyaoTerm('世爻')):line.ying?textHtml(liuyaoTerm('应爻')):'—'}</td><td>${line.moving?t('liuyao.moving'):t('liuyao.still')}<small>${textHtml(liuyaoTerm(line.strength.label))}</small></td><td><details><summary>${t('liuyao.relations')}</summary><ul>${line.relations.length?line.relations.map(liuyaoRelationMarkup).join(''):`<li>${t('liuyao.noRelations')}</li>`}</ul><p class="liuyao-transform">${t('liuyao.transformed')}：${textHtml(line.transformed.ganzhi)} · ${textHtml(liuyaoTerm(line.transformed.element))} · ${textHtml(liuyaoTerm(line.transformed.relative)||'—')}${line.transformed.relations?.length?` · ${textHtml(line.transformed.relations.map(liuyaoTerm).join(language==='en'?', ':'، '))}`:''}</p></details></td></tr>`).join('');
  const useOptions=chart.yongShen.candidates.map(candidate=>`<option value="${attributeHtml(candidate)}" ${candidate===chart.yongShen.selected?'selected':''}>${textHtml(liuyaoTerm(candidate))}</option>`).join('');
  const hexName=index=>displayHexagramName(index,language,hexagrams[index]?.[2]||'—');
  return `<div class="liuyao-overview"><div><span>${t('liuyao.castTime')}</span><b>${textHtml(dateText)}</b><small>${textHtml(chart.timeZone)}</small></div><div><span>${t('liuyao.pillars')}</span><b>${textHtml(chart.calendar.yearPillar)} · ${textHtml(chart.calendar.monthPillar)} · ${textHtml(chart.calendar.dayPillar)} · ${textHtml(chart.calendar.hourPillar)}</b><small>${t('liuyao.monthDay',{month:chart.calendar.monthBranch,day:chart.calendar.dayBranch})}</small></div><div><span>${t('liuyao.palace')}</span><b>${t('liuyao.palaceValue',{palace:chart.original.palace,element:liuyaoTerm(chart.original.palaceElement)})}</b><small>${t('liuyao.shiYing',{shi:chart.shiYing.shiIndex+1,ying:chart.shiYing.yingIndex+1})}</small></div></div><div class="liuyao-use-shen"><label for="liuyaoYongShen">${t('liuyao.useGod')}</label><select id="liuyaoYongShen">${useOptions}</select><small>${chart.yongShen.source==='manual'?t('liuyao.manualUseGod'):t('liuyao.autoUseGod')}</small></div><div class="liuyao-table-wrap"><table class="liuyao-table"><thead><tr><th>${t('liuyao.position')}</th><th>${t('liuyao.najia')}</th><th>${t('liuyao.relative')}</th><th>${t('liuyao.spirit')}</th><th>${t('liuyao.shiYingShort')}</th><th>${t('liuyao.movementStrength')}</th><th>${t('liuyao.branchRelations')}</th></tr></thead><tbody>${rows}</tbody></table></div><div class="liuyao-derived"><span>${t('liuyao.primary')} ${textHtml(hexName(chart.original.index))} → ${t('liuyao.changed')} ${textHtml(hexName(chart.changed.index))}</span><span>${t('liuyao.mutual')} ${textHtml(chart.relations.mutual>=0?hexName(chart.relations.mutual):'—')} · ${t('liuyao.opposite')} ${textHtml(chart.relations.opposite>=0?hexName(chart.relations.opposite):'—')} · ${t('liuyao.inverse')} ${textHtml(chart.relations.inverse>=0?hexName(chart.relations.inverse):'—')}</span></div><details class="liuyao-terms"><summary>${t('liuyao.terms')}</summary><p>${t('liuyao.termsBody')}</p></details></div>`;
}
function renderLiuyaoReading(chart){ensureReadingModes();const target=$('#liuyaoReadingContent');if(!target)return;target.innerHTML=liuyaoMarkup(chart);target.querySelector('#liuyaoYongShen')?.addEventListener('change',event=>{if(!currentReading?.liuyao)return;currentReading.liuyao=applyYongShenOverride(currentReading.liuyao,event.target.value);renderLiuyaoReading(currentReading.liuyao);updateHistoryLiuyao(currentReading.liuyao)});}
function updateHistoryLiuyao(chart){if(!castState.sessionId||!chart)return;const records=loadHistory(),record=records.find(item=>item.id===castState.sessionId);if(!record)return;record.liuyao=chart;record.updatedAt=new Date().toISOString();persistHistory(records);syncCloudRecord(record)}
function showResult(){
  ...
  try{currentReading.liuyao=buildLiuyaoChart({lines:castState.lines,castAt:castState.castAt,timeZone:castState.timeZone,question:castState.question,category:currentReading.local?.category||'general'})}catch(error){currentReading.liuyao=null;console.warn('六爻纳甲排盘未能生成',error)}
  ensureReadingModes();
  const language=getLanguage(),positions=Array.from({length:6},(_,index)=>liuyaoLineLabel(index+1)),originalLines=source?.lines||[],changedFocus=rule.fromChanged?rule.primary.map(index=>changedSource?.lines?.[index]).filter(Boolean):[],principleText=hexagram[3]||'先观其象，再察其时位，最后回到当下可以采取的行动。';
  const lineRows=castState.lines.map((line,index)=>`<div class="result-line-row"><b>${positions[index]}</b><span>${line.value===6?'⚋ ×':line.value===9?'⚊ ○':line.value===8?'⚋':'⚊'}</span><small>${lineName(line.value)}${line.changes?.length?` · ${line.changes.map(change=>change.remaining).join(' → ')} ÷ 4`:''}</small></div>`).join('');
  $('#resultCard').classList.remove('hidden');$('#resultPrimary').textContent=hexagram[1];$('#resultChanged').textContent=changed[1];$('#resultName').textContent=`${displayHexagramName(idx,language,hexagram[2])} → ${displayHexagramName(changedIndex,language,changed[2])}`;$('#resultSummary').textContent=moving.length?t('reading.moving',{lines:moving.map(index=>positions[index]).join(language==='zh-CN'?'、':language==='fa'?'، ':', ')}):t('reading.still');$('#stageHint').textContent=t('reading.focus');$('#readingPanel').classList.remove('hidden');$('#readingTitle').textContent=`${displayHexagramName(idx,language,hexagram[2])} → ${displayHexagramName(changedIndex,language,changed[2])}`;$('#readingQuestion').textContent=t('reading.question',{question:castState.question});$('#readingFocus').textContent=t('reading.focus');
  ...
  renderLiuyaoReading(currentReading.liuyao);
  saveReadingHistory(idx,changedIndex,moving,currentReading.liuyao);
...
function confirmQuestion(){
  ...
  let castTime;
  try{castTime=readCastTimeForm()}catch(error){$('#castTimeFeedback').textContent=error.message||'起卦时间无效，请检查日期、时间和时区。';$('#castTimeFeedback').classList.add('invalid');return}
  $('#castTimeFeedback').classList.remove('invalid');
  castState.question=$('#questionInput').value.trim();castState.sessionId=newSessionId();castState.confirmed=true;castState.prepared=false;castState.castAt=castTime.iso;castState.timeZone=castTime.timeZone;
  $('#questionInput').readOnly=true;$('#consultationGuide').classList.add('confirmed');$$('[data-cast-mode]').forEach(button=>button.disabled=true);$('#confirmQuestion').disabled=true;$('#confirmQuestion').textContent='问题已确认';$('#lockedQuestion').textContent=`所问：${castState.question}`;$('#lockedQuestion').classList.remove('hidden');renderCastTimeControls(castTime);$('#ritualTitle').textContent='五十策已备 · 请亲手虚一';paintStalks('leftStalks',50);paintStalks('rightStalks',0);paintStalks('taijiStalks',0);paintStalks('hangStalks',0);paintStalks('discardStalks',0);setTotal(50);setProcedure('先从五十策中取出一策不用','此一策象太极，不再参与其后的十八变','');updateCastButton();document.querySelector('.ritual-panel').scrollIntoView({behavior:'smooth',block:'start'});saveCast()
}
function restoreCast(){
  ...
  let restoredTime;
  try{restoredTime=normalizeCastTime({value:saved.castAt||new Date(),timeZone:saved.timeZone||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'})}catch{restoredTime=deviceCastTime()}
  castState={lines,working:false,runId:0,ritual:saved.ritual||null,confirmed:true,prepared:Boolean(saved.prepared),question:saved.question.trim(),sessionId:typeof saved.sessionId==='string'&&saved.sessionId?saved.sessionId:newSessionId(),mode:saved.mode==='quick'?'quick':'complete',castAt:restoredTime.iso,timeZone:restoredTime.timeZone};
  setCastMode(castState.mode,true);
  if(source!=='guanxiang-cast-v3')saveCast();
  $('#questionInput').value=castState.question;$('#questionInput').readOnly=true;$('#consultationGuide').classList.add('confirmed');$('#confirmQuestion').disabled=true;$('#confirmQuestion').textContent='问题已确认';$('#lockedQuestion').textContent=`所问：${castState.question}`;$('#lockedQuestion').classList.remove('hidden');renderCastTimeControls(restoredTime);$('#lineCount').textContent=`${castState.lines.length} / 6 爻`;renderRecords();
...
function resetCast(){
  ...
  castState={lines:[],working:false,runId:castState.runId+1,ritual:null,confirmed:false,prepared:false,question:'',sessionId:'',mode,castAt:'',timeZone:''};
...
function toggleLanguageMenu(menu){
  ...
  const activeReadingMode=$('[data-reading-result-mode].active')?.dataset.readingResultMode||'classic'
  let castTimeMeta=null
  try{castTimeMeta=castState.confirmed&&castState.castAt?normalizeCastTime({value:castState.castAt,timeZone:castState.timeZone}):readCastTimeForm()}catch{}
  ...
  localizeReadingModes()
  setReadingResultMode(activeReadingMode)
  renderCastTimeControls(castTimeMeta)
  renderBackupReminder()
  updateCastButton()
  translateKnownText(document)
  renderAccountStatus()
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  renderCastTimeControls();
  $('#useCurrentCastTime')?.addEventListener('click',useCurrentCastTime);
  ['#castDate','#castClock','#castTimeZone'].forEach(selector=>$(selector)?.addEventListener('input',()=>$('#castTimeFeedback')?.classList.remove('invalid')));
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const observer=new MutationObserver(()=>{
    if(!['en','fa'].includes(getLanguage())||translationFrame)return;
