...
const PERSIAN_OVERRIDES = {
  ...
  'history.title': 'دفترچه', 'history.description': 'خوانش‌ها روی این دستگاه می‌مانند و پس از ورود با ابر همگام می‌شوند.', 'history.search': 'جست‌وجوی پرسش، هگزاگرام یا یادداشت',
...
const DICTIONARY = {
  'zh-CN': {
    ...
    'history.title': '占问记录', 'history.description': '完成的卦象保存在本机，登录后可同步到云端。',
  ...
  en: {
    ...
    'history.title': 'Journal', 'history.description': 'Completed readings stay on this device and sync to the cloud after you sign in.',
...
Object.assign(DICTIONARY['zh-CN'], {
  'liuyao.classicMode':'经传解读','liuyao.mode':'六爻纳甲','liuyao.laterSystem':'六爻纳甲 · 后世传统体系','liuyao.completeChart':'完整排盘','liuyao.reviewable':'纳甲、六亲、六神与旺衰依据可展开复核',
  'liuyao.castTime':'起卦时间','liuyao.useCurrent':'使用当前时间','liuyao.currentRestored':'已恢复为此设备当前时间。','liuyao.date':'日期','liuyao.time':'时间','liuyao.timeZone':'时区','liuyao.timeHint':'起卦后将按这个时间计算月建、日辰和旺衰。','liuyao.timeLocked':'时间已锁定；月建、日辰和旺衰将以此时刻计算。','liuyao.defaultTime':'默认时间','liuyao.lockedTime':'本次将按已锁定的起卦时间排盘',
  'liuyao.pillars':'干支','liuyao.monthDay':'月建 {month} · 日辰 {day}','liuyao.palace':'卦宫','liuyao.palaceValue':'{palace}宫 · {element}','liuyao.shiYing':'世爻 {shi} · 应爻 {ying}','liuyao.useGod':'用神（可手动调整）','liuyao.autoUseGod':'按问题类型自动推荐 · 用神不是唯一断法','liuyao.manualUseGod':'已手动选择 · 用神不是唯一断法',
  'liuyao.position':'爻位','liuyao.najia':'纳甲','liuyao.relative':'六亲','liuyao.spirit':'六神','liuyao.shiYingShort':'世应','liuyao.movementStrength':'动静 / 旺衰','liuyao.branchRelations':'生克冲合刑害','liuyao.relations':'关系','liuyao.noRelations':'暂无显著关系','liuyao.transformed':'变爻','liuyao.moving':'动','liuyao.still':'静',
  'liuyao.primary':'本卦','liuyao.changed':'变卦','liuyao.mutual':'互卦','liuyao.opposite':'错卦','liuyao.inverse':'综卦','liuyao.terms':'术语说明','liuyao.termsBody':'纳甲：把天干地支配置到六爻；六亲：父母、兄弟、子孙、妻财、官鬼；六神：按日干起青龙顺排；世应：区分卦中主客位置；月建、日辰：按起卦时刻的节气月和干支日计算；旺衰：显示月建、日辰与本爻五行的依据。',
  'liuyao.unavailable':'六爻排盘暂不可用','liuyao.unavailableHint':'经典经传解读仍然可以正常查看。','liuyao.history':'六爻纳甲 · 历史排盘','liuyao.historyTitle':'按起卦时刻复原','liuyao.legacyMissing':'这条旧记录创建时尚未生成六爻纳甲排盘，经传解读仍完整保留。'
});
Object.assign(DICTIONARY.en, {
  'liuyao.classicMode':'Classic reading','liuyao.mode':'Liuyao Najia','liuyao.laterSystem':'Liuyao Najia · Later traditional system','liuyao.completeChart':'Complete chart','liuyao.reviewable':'Expand each line to review Najia, Six Relations, Six Spirits, strength, and branch relations.',
  'liuyao.castTime':'Casting time','liuyao.useCurrent':'Use current time','liuyao.currentRestored':'Restored to the current time on this device.','liuyao.date':'Date','liuyao.time':'Time','liuyao.timeZone':'Time zone','liuyao.timeHint':'Month commander, day branch, and strength will use this time.','liuyao.timeLocked':'Time locked; the chart uses this instant for month, day, and strength.','liuyao.defaultTime':'Device time','liuyao.lockedTime':'This casting time is locked',
  'liuyao.pillars':'Four pillars','liuyao.monthDay':'Month {month} · Day {day}','liuyao.palace':'Palace','liuyao.palaceValue':'{palace} palace · {element}','liuyao.shiYing':'Self line {shi} · Responding line {ying}','liuyao.useGod':'Focus relation (editable)','liuyao.autoUseGod':'Suggested from the question type; it is not the only valid focus.','liuyao.manualUseGod':'Selected manually; it is not the only valid focus.',
  'liuyao.position':'Line','liuyao.najia':'Najia','liuyao.relative':'Six Relation','liuyao.spirit':'Six Spirit','liuyao.shiYingShort':'Self / response','liuyao.movementStrength':'Change / strength','liuyao.branchRelations':'Generating, control, clash, combine, punishment, harm','liuyao.relations':'Relations','liuyao.noRelations':'No highlighted relation','liuyao.transformed':'Transformed','liuyao.moving':'Moving','liuyao.still':'Still',
  'liuyao.primary':'Primary','liuyao.changed':'Relating','liuyao.mutual':'Nuclear','liuyao.opposite':'Opposite','liuyao.inverse':'Inverse','liuyao.terms':'Terms','liuyao.termsBody':'Najia assigns stems and branches to the six lines. Six Relations are Parent, Sibling, Child, Wealth, and Officer. Six Spirits start from the day stem. Self and Response mark the subject and counterpart. Month and day come from the casting time; strength lists the evidence used for each line.',
  'liuyao.unavailable':'Liuyao chart unavailable','liuyao.unavailableHint':'The classic reading remains available.','liuyao.history':'Liuyao Najia · Saved chart','liuyao.historyTitle':'Restored from the casting time','liuyao.legacyMissing':'This older record predates Liuyao charts. Its classic reading remains intact.'
});
Object.assign(DICTIONARY.fa, {
  'liuyao.classicMode':'تفسیر متون کلاسیک','liuyao.mode':'ناجیای شش‌خطی','liuyao.laterSystem':'ناجیای شش‌خطی · نظام سنتی متأخر','liuyao.completeChart':'جدول کامل','liuyao.reviewable':'برای بررسی ناجیا، شش نسبت، شش روح، قوت و روابط شاخه‌ها هر خط را باز کنید.',
  'liuyao.castTime':'زمان فال‌گیری','liuyao.useCurrent':'استفاده از زمان فعلی','liuyao.currentRestored':'زمان فعلی این دستگاه بازیابی شد.','liuyao.date':'تاریخ','liuyao.time':'زمان','liuyao.timeZone':'منطقه زمانی','liuyao.timeHint':'ماه، روز و قوت خطوط بر پایه این زمان محاسبه می‌شود.','liuyao.timeLocked':'زمان قفل شد؛ محاسبه ماه، روز و قوت بر همین لحظه است.','liuyao.defaultTime':'زمان دستگاه','liuyao.lockedTime':'زمان این فال قفل شده است',
  'liuyao.pillars':'چهار ستون','liuyao.monthDay':'ماه {month} · روز {day}','liuyao.palace':'کاخ','liuyao.palaceValue':'کاخ {palace} · {element}','liuyao.shiYing':'خط خود {shi} · خط پاسخ {ying}','liuyao.useGod':'رابطه کانونی (قابل تغییر)','liuyao.autoUseGod':'بر پایه نوع پرسش پیشنهاد شده و تنها انتخاب معتبر نیست.','liuyao.manualUseGod':'دستی انتخاب شده و تنها انتخاب معتبر نیست.',
  'liuyao.position':'خط','liuyao.najia':'ناجیا','liuyao.relative':'شش نسبت','liuyao.spirit':'شش روح','liuyao.shiYingShort':'خود / پاسخ','liuyao.movementStrength':'تغییر / قوت','liuyao.branchRelations':'زایش، مهار، برخورد، پیوند، کیفر و آسیب','liuyao.relations':'روابط','liuyao.noRelations':'رابطه برجسته‌ای نیست','liuyao.transformed':'خط دگرگون','liuyao.moving':'متحرک','liuyao.still':'ثابت',
  'liuyao.primary':'هگزاگرام اصلی','liuyao.changed':'هگزاگرام پیوسته','liuyao.mutual':'هسته‌ای','liuyao.opposite':'متضاد','liuyao.inverse':'وارونه','liuyao.terms':'واژه‌ها','liuyao.termsBody':'ناجیا ساقه‌ها و شاخه‌ها را به شش خط می‌دهد. شش نسبت شامل والد، هم‌رتبه، فرزند، ثروت و مقام است. شش روح از ساقه روز آغاز می‌شود. خطوط خود و پاسخ دو سوی موضوع را نشان می‌دهند. ماه و روز از زمان فال به دست می‌آیند و قوت، دلیل هر خط را نشان می‌دهد.',
...
Object.assign(TEXT_MAP, {
 ...
 '完成的卦象保存在本机，登录后可同步到云端。':'Completed readings stay on this device and sync to the cloud after you sign in.',
